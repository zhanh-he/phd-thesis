...
For training, we use the MAESTRO v3.0.0 dataset~\cite{maestro} with its default train, validation, and test split.
...
Table~\ref{tab:scoreinf_smd} summarizes the results.
...
\begin{table}[t]
    ...
    \bottomrule
    \end{tabular}
    \caption{Comparison on the SMD dataset. All models are trained only on the MAESTRO training split. Recall$_{10\%}$ results of hFT-Transformer and Transkun v2 are obtained from the MIREX2024: Polyphonic Transcription challenge.\tablefootnote{\url{https://music-ir.org/mirex/wiki/2024:Polyphonic_Transcription_Results}}}
