...
\end{equation}
where $\Psi$ denotes loudness in sones and $I_0$ is the reference intensity \cite{backus1977acoustical}. In MIR implementations, however, the more practical route is usually to go through loudness level in phons and then convert to sones. A widely used piecewise approximation is
\begin{equation}
S(L_N)=
\begin{cases}
...
\begin{table*}[h!]
\centering
\caption[Score-dynamics vocabulary and loudness-related scales]{Illustrative correspondences between score-dynamics vocabulary and loudness-related scales. These values are reader-oriented anchors rather than deterministic conversions from notation to acoustics.}
...
\begin{figure}[h!]
  ...
  \caption{Equal loudness contours comparing ISO226:2003 (red) and the measurements of Robinson and Dadson in the mid 1950s (black) \cite{equal_loudness}.}
